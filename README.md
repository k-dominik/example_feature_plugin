# example_feature_plugin

Example plugin to show rendered state of cookiecutter.
...
Open the project, when clicking on _Select Features_, the **example_feature_plugin** plugin should show up.
...
The `example_feature_plugin` folder holds a template that you can use to implement your feature plugin.
The file `example_features.py` and `example_features.yapsy-plugin` are the only files that you will need to edit.

* `example_features.yapsy-plugin`: this holds the metadata that is used by the plugin manager that is used in ilastik `yapsy`.
* `example_features.py`: The class `ExampleFeatures` is the template class that you need to edit in order to add your feature.
